...
这是一个好问题。我们可以在使用内联缓存的代码上调用.call或者.apply。这样的代码可以工作，现代的JIT（即时编译器）也不会像原来那样不支持这种写法，但是出于对以下几个原因的考虑，我们还是不推荐使用这样的写法。

首先，这些专用的内联缓存有确定的参数列表以及确定的参数类型。在这种情况写，每个MethodSignature的实例都是一个确定类型的参数列表。在调用的时候，运行引擎将方法名和这样一个MethodSignature结合起来去调用一个特定方法的某个特定重载函数。这同时也意味着，一个编译后的应用对于一个独一无二的参数组合只有一个MethodSignature实例。所以任何调用路径经过这个参数和重载方法的组合将通过这个MethodSignature的实例来进行调用。

共享MethodSignature的缺点在于对内联缓存的要求更高了，因为我们不能硬编码函数名称。但这样做的优点是，MethodSignature的信息重复使用可以节省相当一部分内存空间，并且调用这段MethodSignature的代码将会达到相当高的使用频率，这样高的使用频率让这段代码得到充分的优化。

有了MethodSignature提供的确定的参数列表，JIT（即时编译器）就可以不用去猜或者推断类型。一旦JIT观测到MethodSignature对象多次调用，它就知道所有的类型都是已经确定的。（有一个例外情况是，当你给方法传一个对象的时候，参数类型有可能会变化。）

在这种情况下，JIT（即时编译器）更容易产生高效的代码，因为有更少的类型检查，并且不会像原来那样频繁触发重新编译。

考虑到我们需要使用MethodSignature提供的类型信息并且确定即将调用的函数，那么同时考虑我们能都沿着调用链传递类型信息就是很合理的。
如果我们在MethodSignature内部使用Function.call和Function.apply去调用实际的方法体，那么我们就会很高效地抛出类型信息。对于JIT（即时编译器）来说，收集这些方法的参数类型信息并不是很容易，因为我们习惯于调用各种方法，甚至apply函数的参数还会在数组内部。

更糟糕的是，当JIT想要内联方法的时候，call和apply方法仍然有可能会抛出一个问题，因为它们不知道方法调用的具体目标。（？）
正如我们前面提到的，JIT使用内联缓存是通过调用一个名字确定的方法（methodSource），这样函数内联会很方便。
在这个方面，call和apply方法的表现甚至比methodSource更糟糕，因为不仅仅目标方法未知，而且我们还可能需要进行特殊操作来处理参数数量或者类型不匹配的情况。
（如果在SpiderMonkey分析器中查看call和apply方法的使用情况，事实上，我们会看到JS在运行时把相当多的时间都放在处理参数列表上了）

以上论述的所有原因都给我们足够的理由尝试对于每个MethodSignature实例生成一个特定的函数负责用给定的名字调用给定的方法。
这就是内联缓存的作用，以上论述的一切使得下面的代码可以正常运行：
```
var adder = ...;
...
在实际上，很可能此处产生的所有代码都会被内联并且需要很少的类型检查。

## 现在，让我们来看看源代码！

[源代码](https://github.com/sq/JSIL/blob/db878c56fa12b7d8427fa84e4ce79b477a4a728e/Libraries/JSIL.Core.js#L7008)的第7008-7413行包含了我实现内联缓存的代码，有注释哦~

## 终章

如果想尝试类似这样的复杂优化操作并且在细节处调试javascript性能的时候，你就必须不厌其烦地重复测试。

对于大多数JSIL的优化，我都会进行一系列从每次提交都会用于测试的真实应用中提取出的的微基准测试来确保性能不会有倒退的表现。
在这里，我定期运行使用JSIL最新版本编译的实际应用程序，检查它们的整体帧率，查看配置文件，看看是否出现了任何新的热代码可以进行优化。
有时我在各种浏览器上运行我的性能测试用例，比如Firefox和Chrome的发布版和开发版、IE，看看某个特定的浏览器是有了显著的改进还是倒退。这样测试通常会发现针对特定浏览器引擎或js引擎的bug。

如果您的目标是为库或应用程序编写尽可能快的JavaScript代码，那么您可能需要做到同样的深度，然而对于许多应用程序来说，这是不值得的——所以不要疯狂地对一开始并不特别慢的东西进行微优化。

## 感谢阅读

这个文章只是一个草稿，如果您有建议或者想法请通过[推特](http://twitter.com/antumbral)或者邮箱与我沟通
